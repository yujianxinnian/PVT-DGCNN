...
from torch_geometric.nn import MessagePassing,DynamicEdgeConv
...
class DEConvGNN_FFN(nn.Module):
    def __init__(self, in_channels, out_channels, k=6, graph_net_layers=3):
        super(DEConvGNN_FFN, self).__init__()
        ...
        for i in range(graph_net_layers):
            ...
            nn.BatchNorm1d(2*in_channels,eps=1e-3, momentum=0.01),
            nn.ReLU(),
            ...
            nn.BatchNorm1d(out_channels,eps=1e-3, momentum=0.01),)
            self.graph_nets.append(DynamicEdgeConv(nn=mlp,k=self.k,num_workers=2))
...
class MLP_VSA_Layer(nn.Module):
    def __init__(self, dim, n_latents=8,k_gnn=6,gnn_layers=3):
        ...
        self.gnn_ffn = DEConvGNN_FFN(conv_dim,conv_dim,k_gnn,gnn_layers)
    ...
    def forward(self, inp, inverse, coords, bev_shape):
        ...
        # 体素间信息交换 
        batch = coords[:,0].contiguous()
        h = self.gnn_ffn(F.relu(x_),batch)
