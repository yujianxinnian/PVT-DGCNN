...
from torch_geometric.nn import MessagePassing,PointGNNConv
...
class Voxt_Sgcnn_Two_Stage(VFETemplate):
...
class StaticEdgeConv(EdgeConv):
    ...
    def forward(self, x, coords):
        ...
        edge_index = knn_graph(coords[:,1:].contiguous(), k=self.k, batch=coords[:,0].contiguous(), loop=True)
...
class GNN_FFN(nn.Module):
    def __init__(self, conv_dim=48,k_gnn=3,graph_net_layers=3):
        '''
        ...
        for i in range(graph_net_layers):
            self.graph_nets.append(StaticEdgeConv(conv_dim,conv_dim,k_gnn))
...
class MLP_VSA_Layer(nn.Module):
    def __init__(self, dim, n_latents=8,k_gnn=6,gnn_layers=3):
        ...
        self.gnn_ffn = GNN_FFN(conv_dim,k_gnn,gnn_layers) 
    ...
    def forward(self, inp, inverse, coords, bev_shape):
        ...
        h = self.gnn_ffn(F.relu(x_),coords)
